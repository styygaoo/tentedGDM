...
class NYU_Testset_Extracted(Dataset):
    def __init__(self, root, resolution='full'):
        ...
        self.files = sorted(os.listdir(self.root), key=last_3chars)
...
class NYU_Testset(Dataset):
    def __init__(self, zip_path):
        ...
        data = {name: input_zip.read(name) for name in input_zip.namelist()}        # define a dictionary.

        self.rgb = torch.from_numpy(np.load(BytesIO(data['image.npy']))).type(torch.float32) #Range [0,1]
        self.depth = torch.from_numpy(np.load(BytesIO(data['depth.npy']))).type(torch.float32) #Range[0, 10]
