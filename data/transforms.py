...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if self.test:
            ...
            """
            # print("depth: ", depth)
            image = np.array(image).astype(np.float32) / 255.0              # The astype() method of numpy.ndarray can convert the data type dtype.
            depth = np.array(depth).astype(np.float32) #  / self.maxDepth #Why / maxDepth?
            # print("image: ", image)
            # print("depth: ", depth)
            image, depth = transformation(image), transformation(depth)
            # print("depth after tansformation: ", depth)
            # print("image after tansformation: ", image)
        else:
            #Fix for PLI=8.3.0
